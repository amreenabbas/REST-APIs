...
def add_user():
    ...
    if len(str(user_id)) > 50:
	return Response('Id should not exceed 50 characters', status=400) 
...
def crgroup():
    ...
    if group_name != None:
        ...
        if len( str(group_name)) > 50:
	    return Response('Group name should not exceed 50 characters', status=400) 
...
def reserve():
    ...
    valid_frequency = "hourly daily weekly null"
    if frequency != None:
	index_of_frequency = valid_frequency.find(str(frequency))
	if (frequency.isalpha() == False) | (index_of_frequency == -1):
	    return Response('frequency should be either hourly, daily or weekly', status=400) 
    duration = body.get("duration")
    duration_seconds = 0
    res_start_time = body.get("start-time")
    res_end_time = body.get("end-time")
    if (res_end_time == None) | (res_start_time == None):
	return Response('start time and end time not given', status=400) 
    try:
        reservation_start_time_obj = datetime.strptime(res_start_time, '%Y-%m-%d %H:%M:%S')
        reservation_end_time_obj = datetime.strptime(res_end_time, '%Y-%m-%d %H:%M:%S')
        ...
        reservation_end_time = time.mktime(reservation_end_time_obj.timetuple())
           ...
           return Response('invalid date time format', status=400)
    time_now = datetime.now()
    now_seconds = time.mktime(time_now.timetuple())
    if (reservation_start_time > reservation_end_time) | (reservation_start_time < now_seconds):
	return Response('invalid date time', status=400)
    if frequency == None:
	...
	if duration == None:
	    ...
	    reservation_time = reservation_end_time - reservation_start_time
	    if duration_seconds > reservation_time:
		return Response('duration exceeds reservation time', status=400)
    if verify_apikey(user_id, user_apikey):
	if gid != None:		
	    flag1 = 0
	    flag2 = 0
	    sql_select_query = """select device_list from groups where uid = %s and gid = %s"""
			...
			active_device_list = [device for device in device_list if device in record[9]]
...
def upreserve(rid):
	...
	if verify_apikey(user_id, user_apikey):
		...
		if duration != None:
	        ...
	        if device_list != []:
		    if (device for device in device_list if device in reservation_info[5]) != None:	 	 
			flag1 = 1
		flag2 = 0
	        query = """select device_list from reservations as r, device_state as s where r.rst <= %s and r.ret >= %s and r.rid = s.rid and r.rid != %s"""		    
	        cur.execute(query, (reservation_start_time,reservation_end_time,rid))
...
def configure():
	...
	cid = body.get('cid')
	reservation_info1 = None
	reservation_info2 = None
	if (user_id.isalnum() == False) :
	    return Response('Id should be only alphanumeric', status=400) 
	...
	if verify_apikey(user_id, user_apikey):
		...
		if configurations != None:
		    if did_list != None:
		        cur.execute("""INSERT INTO configure (rid,configurations,type,id) VALUES (%s,%s,%s,%s)""",(user_info[0],json.dumps(configurations),'sync',json.dumps(did_list)))
    		        connection.commit()	
		        return Response('configurations stored', status=200)
		    else:
			return Response('device list missing', status=400)
		else:
		...
		if (rid.isalnum() == False) :
	        ...
	        cur.execute(query, (rid,user_id))
    	        reservation_info1 = cur.fetchone()
	    if reservation_info1 == None:
	        if cid != None:
		    if (cid.isalnum() == False) :
	    	        return Response('cid should be only alphanumeric', status=400) 
		    query = """select * from reservations_info where cid = %s and uid = %s"""		    
	            cur.execute(query, (cid,user_id))
    	            reservation_info2 = cur.fetchone()
	    if (reservation_info1 == None) & (reservation_info2 == None):
		return Response('invalid rid or cid', status=400)
	    reservation_info = reservation_info1 if reservation_info1 != None else reservation_info2
		...
		return Response('configurations already created for given rid or cid, you can update it through put method', status=405)	
		    ...
		    if configurations != None:
			    ...
			    for device_list in device_info[1]:
			        flag = all(device in device_list for device in did_list)
			    if flag == True:			
	...
	if verify_apikey(user_id, user_apikey):
	    ...
	    if user_info[3] == 'admin':
		if configurations != None:
		    query = """select * from configure where uid = %s"""		    
	            cur.execute(query, (user_id, ))
    	            configure_info = cur.fetchone()
		    if configure_info != None:
		        query = """UPDATE configure set configurations = %s where rid = %s and type = %s"""
    		        cur.execute(query, (json.dumps(content),user_info[0],'sync'))
                        connection.commit()
		        return Response('configuration stored', status=200)
		    else:
			return Response('configuration to be updated not stored yet', status=400)
		else:
		    return Response('configurations missing', status=400)
	    if (rid == None) & (cid == None):
		return Response('rid or cid missing', status=400)
	    ...
	    if (record1 == None) & (record2 == None):
		return Response('invalid rid or cid', status=400)
	    record = record1 if record1 != None else record2
	    query = """select * from configure where rid = %s and type = %s"""
    	    cur.execute(query, (record[0],'sync'))
            configuration_record = cur.fetchone()
	    if configuration_record == None:
		return Response('configurations not created for given rid or cid, you can create it through post method', status=404)	
	    else:
	        if configurations != None:
		    query = """UPDATE configure set configurations = %s where rid = %s and type = %s"""
	...
	if verify_apikey(user_id, user_apikey):
		...
		if configure_info != None:
		    query = """Delete from configure where type = %s  and rid = %s"""
               	    cur.execute(query, ( 'sync',user_info[0]))
                    connection.commit()
		    return Response('configuration deleted', status=200)
	        else:
		    return Response('configuration not found to be deleted', status=400)
	    if (rid == None) & (cid == None):
		return Response('rid or cid missing', status=400)
	    elif rid != None:
		if (rid.isalnum() == False) :
	    ...
	    if record1 == None:
		    ...
		    query = """select * from reservations_info where cid = %s and uid = %s"""		    
	            cur.execute(query, (cid,user_id))
    	            record2 = cur.fetchone()
	    if (record1 == None) & (record2 == None):
		return Response('invalid cid or rid',status=400)
	    record = record1 if record1 != None else record2
	    query = """select * from configure where rid = %s and type = %s"""
    	    cur.execute(query, (record[0],'sync'))
            configuration_record = cur.fetchone()
...
def write():
	...
	if verify_apikey(user_id, user_apikey):
		...
		if content != None:
		    if did_list != None:
		        cur.execute("""INSERT INTO write_op (rid,content,type,id) VALUES (%s,%s,%s,%s)""",(user_info[0],json.dumps(content),'sync',json.dumps(did_list)))
    		        connection.commit()	
	...
	if verify_apikey(user_id, user_apikey):
	    ...
	    if user_info[3] == 'admin':
		if content != None:
		    query = """select * from write_op where uid = %s"""		    
	            cur.execute(query, (user_id, ))
			...
			return Response('content to be updated not written yet', status=400)
		else:
		    return Response('content missing', status=400)
	    if (rid == None) & (cid == None):
		return Response('rid or cid missing', status=400)
		...
		query = """select * from write_op as c, reservations_info as r where c.rid = r.rid and c.type = %s and r.rid = %s and r.cid != 'null' and r.uid = %s"""
    		cur.execute(query, ('sync',record[0],user_id))
                write_record = cur.fetchone()
		if write_record != None:
		    query = """UPDATE write_op set content = %s where rid = %s and type = %s"""
	...
	if verify_apikey(user_id, user_apikey):
		...
		if write_info != None:
		    ...
		    return Response('content to be deleted not found', status=400)
	    if (rid == None) & (cid == None):
		return Response('rid or cid missing', status=400)
	    elif rid != None:
		if (rid.isalnum() == False) :
...
def opstat():
	...
	if rid != None:
	    if rid.isalnum() == False:
		return Response('rid should be only alphanumeric', status=400)
	    query = """select * from device_state as s, reservations as r, reservations_info as i where s.rid = r.rid and r.rid = %s and i.rid = r.rid and i.uid = %s"""
            cur.execute(query, (rid,user_id))
            record = cur.fetchone()
	    if record == None:	
	        return Response('invalid rid', status=400)
	    if did != None:
...
def devstat():
	...
	if rid != None:
	    if rid.isalnum() == False:
		return Response('rid should be only alphanumeric', status=400)
	    query = """select * from reservations_info where rid = %s and uid = %s"""
	    cur.execute(query, (rid,user_id))
            reservation_record = cur.fetchone()
	    if reservation_record == None:
		return Response('invalid rid', status=400)
	    query = """select * from configure as c, device_state as r, reservations_info as i where r.rid = c.rid and c.rid = %s and i.rid = r.rid and i.uid = %s"""
            cur.execute(query, (rid,user_id))
            record = cur.fetchone()
            if record == None:	
...
def opchannel():
	...
	query = """select * from reservations as r, reservations_info as i, device_state as s where r.rid = i.rid and r.rid = s.rid and i.uid = %s and r.rid = %s"""
		...
		flag = False
		for row in records:
	            lst3 = [value for value in reservation_info[13] if value in row[1]] 
		    if lst3 != []:
			flag = True
    	        if flag == False:	
...
def clchannel():
	...
	if (rid == None) & (cid == None):	
	    ...
	    if (rid.isalnum() == False) :
	    	return Response('rid should be only alphanumeric', status=400) 
	    query = """select * from reservations_info as i,reservations as r where i.uid = %s and i.rid = %s  and i.cid != 'null' and i.rid = r.rid"""
            cur.execute(query, (user_id,rid))
    	    reservation_info2 = cur.fetchone()
	if reservation_info2 == None:
	    if cid != None:
	        if (cid.isalnum() == False) :
	    	    return Response('cid should be only alphanumeric', status=400) 
	        query = """select * from reservations_info as i, reservations as r where i.uid = %s and i.cid = %s and i.rid = r.rid"""
                cur.execute(query, (user_id,cid))
    	        reservation_info1 = cur.fetchone()
        if (reservation_info2 != None) | (reservation_info1 != None):
	    reservation_info = reservation_info2 if reservation_info2 != None else reservation_info1
	    if reservation_info[6] == reservation_info[8]:
	    	sql_delete_query = """Delete from reservations_info where rid = %s"""
            	cur.execute(sql_delete_query, (reservation_info[0], ))
            	connection.commit()
	    else:
		sql_update_query = """update reservations_info set cid = %s where rid = %s"""
            	cur.execute(sql_update_query, ('null',reservation_info[0] ))
            	connection.commit()
	    return Response('channel closed', status=200)
	else:
	    return Response('cid or rid invalid or time outside reservation time', status=400)
    else:
	return Response('invalid user id or apikey', status=401)
	...
	if record2 == None:
	    if cid != None:
	        ...
	        query = """select * from reservations_info where uid = %s and cid = %s"""
                cur.execute(query, (user_id,cid))
    	        record1 = cur.fetchone()
        if (record2 != None) | (record1 != None):
	    record = record2 if record2 != None else record1
	    query = """select * from reservations where rid = %s"""
...
def opfunc():
	...
	for record in records:
	    ...
	    flag = False
	    for row in records:
	        lst3 = [value for value in record[1] if value in row[1]] 
		if lst3 != []:
		    flag = True
    	    if flag == False:	
	        query = """UPDATE reservations set state = %s where rid = %s"""
		...
		query = """select * from configure where rid = %s and type = %s"""
    	    	...
    	    	configure_info = cur.fetchone()
		if configure_info != None:
		    query = """UPDATE configure set type = %s where rid = %s"""
    		    cur.execute(query, ('sync',record[0]))
                    connection.commit()
	    else:
		return Response('high priority task executing', status=409)
...
def clfunc():
	...
	for record in records:
		...
		if frequency == "weekly":
		    ...
		    end_time = start_time + record[7]
		if (start_time > record[2]) | (end_time > record[2]):
		    sql_delete_query = """Delete from write_op where rid = %s"""
                    cur.execute(sql_delete_query, (record[0], ))
                    connection.commit()
	            sql_query = """Delete from configure where rid = %s"""
		    ...
		    query = """UPDATE reservations set state = %s, st = %s, et = %s where rid = %s"""
    		    cur.execute(query, ('inactive',start_time,end_time,record[0]))
                    connection.commit()
		    query = """UPDATE reservations_info set cid = %s where rid = %s"""
    		    cur.execute(query, ('null',record[0]))
                    connection.commit()
    else:
	return Response('', status=200)
